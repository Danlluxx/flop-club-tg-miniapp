...
SELECT pg_advisory_xact_lock(hashtext('2026-07-16-flop-butterfly-rating-fix'));
...
BEGIN
  ...
  IF tournament_count <> 1 THEN
    RAISE EXCEPTION 'Expected exactly one Flop Butterfly tournament on 2026-07-16 Barnaul date, found %', tournament_count;
  END IF;
END $$;
...
INSERT INTO _manual_results (finish_place, points, percent, lookup_values) VALUES
  (1, 5400, 45, ARRAY['rondo', 'rondoilya']),
  (2, 3240, 27, ARRAY['Yakub Aliev', 'Yakub', 'yakubahlli']),
  (3, 2160, 18, ARRAY['👾такиживу', 'такиживу']),
  (4, 1200, 10, ARRAY['DK', 'danlluxx']),
  (5, 0, 0, ARRAY['bbr🃏', 'bbr', 'nbobr']),
  (6, 0, 0, ARRAY['Bq']),
  (7, 0, 0, ARRAY['Бонзу Пипенподол Апсокополис III', 'wsemprivetik']),
  (8, 0, 0, ARRAY['David', 'davidmane']),
  (9, 0, 0, ARRAY['TOPAT', 'topat']),
  (10, 0, 0, ARRAY['Фёдор Иванович Депутат', 'Федор Иванович Депутат', 'Фёдор Депутат', 'Федор Депутат', 't_ador']),
  (11, 0, 0, ARRAY['Baldejnyi', 'Baldejniy', 'baldejnyi']),
  (12, 0, 0, ARRAY['Scalpbets', 'ScalpBets', 'scalpbetsceo']);
...
SELECT
  'manual_reg_20260716_butterfly_' || LPAD(resolved.finish_place::TEXT, 2, '0'),
  resolved.user_id,
...
SELECT
  'manual_rating_20260716_butterfly_' || LPAD(resolved.finish_place::TEXT, 2, '0'),
  (SELECT id FROM _target_tournament),
  resolved.user_id,
  resolved.finish_place,
  resolved.percent,
...
UPDATE "Tournament"
SET
  "entriesCount" = GREATEST(COALESCE("entriesCount", 0), 12),
  status = 'FINISHED',
  "updatedAt" = CURRENT_TIMESTAMP
